library(pheatmap)
data <- read.table('input.list1.deal.addtitle',header=T,row.names= 1)
anno <- read.table('annotation.list',header=T,row.names= 1)
ann_colors <- list(CLASS = c(class1 = "#1B9E77", class0 = "#D95F02"))
pdf('CGI_heatmap.pdf',width=12,height=8)
...
pheatmap(data,cluster_col = col_cluster,
         color = colorRampPalette(c("#D95F02", "white", "#1B9E77"))(60), 
